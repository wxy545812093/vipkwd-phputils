...
class Address{
    ...
    public function smart(string $string, bool $user = true, bool $debug = true):array{
        ...
        if($user){
            // $re = array_merge($re, self::decompose($string));
            $re = array_merge($re, self::decompose($fuzz['street']));
            if(isset($re['name']) && $re['name']){
                $fuzz['street'] = preg_replace("/(\ +)?{$re['name']}(\ +)?/i", '', $fuzz['street'] ?? '');
            }
            if(isset($re['mobile']) && $re['mobile']){
                $fuzz['street'] = preg_replace("/(\ +)?{$re['mobile']}(\ +)?/i", '', $fuzz['street'] ?? '');
            }
            if(mb_strlen($re['text']) <= mb_strlen($fuzz['street'])){
                $fuzz['street'] = $re['text'];
            }
            unset($re['text']);
        }
        $re['address'] = str_replace([
                $re['county'],
                $re['city'],
                $re['province']
            ], "", $fuzz['street'] ?? "" );
        
        $debug && $re['__text'] = $string;
        return $re;
    }
    ...
    private static function fuzz(string $addr):array{
        ...
        $__validate = function($t)use($addr){
            $pos = mb_strpos($addr, $t);
            return $pos !== false &&  $pos < floor((mb_strlen($addr) / 3) * 2);
        };

        if ( $__validate('旗') || $__validate("区") || $__validate("县") ) {

            if (mb_strstr($addr, '旗')) {
                $deep3_keyword_pos = mb_strpos($addr, '旗');
                $a3 = mb_substr($addr, $deep3_keyword_pos - 1, 2);
            } else if (mb_strstr($addr, '区')) {
