...
class Thumb
{
	...
	public function createPlaceholder(string $WxHsize, int $getType = 1, int $format = 1, int $fontSize = 0, string $text = "W x H", string $bgColor = "#666", string $textColor = "#fff", int $angle = 0)
	{
		try {
			...
			$content = ($_text && $_text != "WXH") ? mb_substr($text, 0, intval(ceil($width / 9))) : $width . ' x ' . $height;
...
trait ThumbCore
{
	...
	private function createemptythumbnail(): void
	{
		...
		if ($this->Percentage) {
			if ($thumbwidth > 0) {
				$thumbwidth = intval(($thumbwidth / 100) * $this->size[0]);
			}
			if ($thumbheight > 0) {
				$thumbheight = intval(($thumbheight / 100) * $this->size[1]);
			}
