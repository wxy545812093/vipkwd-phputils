...
class Captcha
{
	 ...
	 * 输出验证码|BASE64
	 *
	 * -e.g: phpunit("Image\Captcha::entry");
	 * 
	 ...
	 * @return array [code=> 1285, data=> imageBase64]
	 */
	static function entry(int $width = 0, int $height = 0, int $length = 4, int $fontSize = 25, bool $noise = true, bool $curve = true): array
	{
		...
		$len = strlen(self::$codeSet) - 1;
		for ($i = 0; $i < self::$length; $i++) {
			$code[$i] = self::$codeSet[mt_rand(0, $len)];
		...
		for ($i = 0; $i < 10; $i++) {
			...
			for ($j = 0; $j < 5; $j++) {
				...
				imagestring(
					...
					self::$codeSet[mt_rand(0, $len)], // 杂点文本为随机的字母或数字
					$noiseColor
				);
			}
		}
