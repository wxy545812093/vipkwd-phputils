...
if(!defined('VIPKWD_EXCEPTION')){ return; }
define('SITE_PATH', "vipkwd_com");
/**
 * 异常处理
 *
...
function handleException($exception) {
	VipkwdException::exception($exception);
}
...
function handleError($errNo, $errStr, $errFile, $errLine) {
	if ($errNo) {
		VipkwdException::error($errStr, true, true, false);
	}
}
...
 *
 */
class VipkwdException {

	public static function error($message, $show = true, $save = true, $halt = true) {
		$trace = self::debugBacktrace();
		list($showTrace, $logTrace) = $trace;
		if ($save) {
	...
	public static function debugBacktrace() {
		...
		foreach ($debugBacktrace as $k => $error) {
			...
			$error['line'] = sprintf("%'.4s", $error['line']);
			$error['line'] = str_replace(".","&nbsp;", $error['line']);
	...
	public static function showError($type, $errorMsg, $phpMsg = '') {
		...
		<body>
		<div id="container">
		<h1>$title Exception/Error</h1>
		EOT;
		if (!empty($phpMsg)) {
			echo "<div class='info'><span>{$errorMsg}</span>";
			if(is_array($phpMsg)){
				$msg_first_key = array_key_first($phpMsg);
				echo " [ {$phpMsg[$msg_first_key]['file']}({$phpMsg[$msg_first_key]['line']}) ]</div>";
			}else{
				echo "</div>";
			}
			echo '<div class="info">';
