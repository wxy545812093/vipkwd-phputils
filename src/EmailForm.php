...
class EmailForm
{
    ...
    {
        $this->contact = new PHP_Email_Form;
    ...
    public function smtp(string $username, string $password, string $host, int $port = 465, string $encryption = 'SSL'): self
    {
        ...
        $this->contact->smtp = [
            ...
            'password' => $password,
            'port' => $port,
            'encryption' => strtolower($encryption) == 'ssl' ? PHPMailer::ENCRYPTION_SMTPS : PHPMailer::ENCRYPTION_STARTTLS,
        ];
        return $this;
    }
    ...
    public function send()
    {
        ...
        if (is_array($this->smtp)) {
            ...
            if (!isset($this->smtp['encryption']))
                $this->smtp['encryption'] = 'tls'; //传输层安全 协议
...
class SMTP
{
    ...
    protected function get_lines()
    {
        ...
        while (is_resource($this->smtp_conn) && !feof($this->smtp_conn)) {
            ...
            $this->edebug('SMTP INBOUND: "' . trim($str ? $str : '') . '"', self::DEBUG_LOWLEVEL);
            $data .= $str;
